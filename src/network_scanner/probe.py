...
async def _tcp_open(ip: str, port: int, timeout: float = 0.45) -> bool:
    writer = None
    try:
...
async def probe_host(ip: str) -> Dict[str, Any]:
    ...
    port_results = await asyncio.gather(*(_tcp_open(ip, port) for port in ports))
    pingable = await ping_task
    open_ports = [port for port, is_open in zip(ports, port_results) if is_open]
    reachable = pingable or bool(open_ports)
